#!/usr/bin/env python3
"""
Simple dashboard for irrigation predictions and monitoring.
"""
...
def load_demo_data():
    ...
    try:
        # Try to load the main project results first
        if os.path.exists("data/processed/test_predictions.csv"):
            # Load main project data
            demo_data = pd.read_csv("data/processed/test_predictions.csv", parse_dates=['date'])
            test_data = pd.read_csv("data/processed/irrigation_test.csv", parse_dates=['date'])

            # Merge predictions with test data
            demo_data = pd.merge(test_data, demo_data, on=['zone_id', 'date'], how='left')
            ...
            demo_data['physics_baseline_mm'] = demo_data['physics_baseline_mm']
            ...
            summary = {
                ...
                'final_mae': abs(demo_data['predicted_irrigation_mm'] - demo_data['irrigation_mm_next_day']).mean(),
                'physics_mae': abs(demo_data['physics_baseline_mm'] - demo_data['irrigation_mm_next_day']).mean(),
                'ml_mae': abs(demo_data['ml_prediction_mm'] - demo_data['irrigation_mm_next_day']).mean(),
                'hybrid_mae': abs(demo_data['hybrid_prediction_mm'] - demo_data['irrigation_mm_next_day']).mean(),
                'fc_violations_prevented': 0,  # Will be calculated
                'under_irrigation_rate': ((demo_data['predicted_irrigation_mm'] - demo_data['irrigation_mm_next_day']) < -1.0).mean(),
                'over_irrigation_rate': ((demo_data['predicted_irrigation_mm'] - demo_data['irrigation_mm_next_day']) > 1.0).mean(),
                'num_features': 90,
        ...
        # Fallback to quick demo data
        elif os.path.exists("data/sample/demo_results.csv"):
            demo_data = pd.read_csv("data/sample/demo_results.csv", parse_dates=['date'])
            ...
            st.error("No demo data found. Please run 'python run_project.py' or 'python quick_demo.py' first.")
            return None, None
...
def create_performance_metrics_plot(summary):
    ...
    metrics = ['Physics Baseline', 'ML Model', 'Hybrid Model', 'Final Safe']
    mae_values = [
        summary['physics_mae'],
        summary['ml_mae'],
        summary['hybrid_mae'],
...
def create_water_usage_plot(df):
    """Create water usage visualization."""
    # Group by zone and sum water usage
    zone_water = df.groupby('zone_id')['predicted_liters'].sum().reset_index()
    
    fig = px.pie(
        zone_water, 
        values='predicted_liters', 
        names='zone_id',
...
def main():
    ...
    # Load data
    demo_data, summary = load_demo_data()
    ...
    with col3:
        st.metric(
            "FC Violations Prevented", 
            f"{summary['fc_violations_prevented']}",
            help="Number of field capacity violations prevented by safety constraints"
        )
    ...
    # Main content tabs
    tab1, tab2, tab3, tab4 = st.tabs(["📈 Predictions", "🌱 Zone Analysis", "⚡ Performance", "📋 Data"])
    ...
    with tab2:
        ...
        with col2:
            ...
            water_fig = create_water_usage_plot(demo_data)
        ...
        total_irrigation = zone_data['predicted_irrigation_mm'].sum()
    ...
    with tab3:
        ...
        # Error analysis
        demo_data['error'] = demo_data['predicted_irrigation_mm'] - demo_data['irrigation_mm_next_day']
        ...
        with col1:
            ...
            fig_error = px.histogram(
                ...
                labels={'error': 'Error (mm)', 'count': 'Frequency'}
            )
        ...
        with col2:
            ...
            fig_scatter = px.scatter(
                demo_data,
                x='irrigation_mm_next_day',
                y='predicted_irrigation_mm',
                color='zone_id',
                title="Actual vs Predicted Irrigation",
                labels={'irrigation_mm_next_day': 'Actual (mm)', 'predicted_irrigation_mm': 'Predicted (mm)'}
            )
            
            # Add perfect prediction line
            max_val = max(demo_data['irrigation_mm_next_day'].max(), demo_data['predicted_irrigation_mm'].max())
        ...
        perf_data = {
            'Metric': ['Physics MAE', 'ML MAE', 'Hybrid MAE', 'Final MAE', 'Under-irrigation Rate', 'Over-irrigation Rate'],
            'Value': [
                f"{summary['physics_mae']:.3f} mm",
                f"{summary['ml_mae']:.3f} mm", 
                f"{summary['hybrid_mae']:.3f} mm",
                f"{summary['final_mae']:.3f} mm",
                f"{summary['under_irrigation_rate']:.1%}",
                f"{summary['over_irrigation_rate']:.1%}"
            ]
        }
